...
def guard(request: Request):
    if request.cookies.get(ADMIN_COOKIE) != 'ok':
...
def admin(request:Request,db:Session=Depends(get_db)):
    ...
    cafe_ready=naver_cafe.configured
...
def naver_login(request: Request):
    ...
    if g:return g
    try:
        return RedirectResponse(naver_cafe.authorization_url(),303)
    except Exception as exc:
        return HTMLResponse(f'<h2>네이버 카페 설정 오류</h2><p>{exc}</p><p><a href="/admin">관리자</a></p>',500)

@app.get('/naver/callback')
def naver_callback(request: Request, code: str = '', state: str = '', error: str = '', error_description: str = ''):
    if error:
        return HTMLResponse(f'<h2>네이버 인증 실패</h2><p>{error_description or error}</p><p><a href="/admin">관리자</a></p>',400)
    if not code or not state:
        return HTMLResponse('<h2>네이버 인증 정보가 없습니다.</h2><p><a href="/admin">관리자</a></p>',400)
    try:
        naver_cafe.exchange_code(code,state)
        return RedirectResponse('/admin',303)
    except Exception as exc:
        return HTMLResponse(f'<h2>네이버 인증 처리 실패</h2><p>{exc}</p><p><a href="/admin">관리자</a></p>',500)
...
def smartstore(product_id:int,request:Request,db:Session=Depends(get_db)):
    ...
    if p:
        try:
            ...
            p.status='SMARTSTORE_ERROR';log(db,'SMARTSTORE_REGISTER',str(exc),p.id,'ERROR')
        db.commit()
    return RedirectResponse('/admin',303)
...
def cafe(product_id:int,request:Request,db:Session=Depends(get_db)):
    ...
    if p:
        try:
            ...
            log(db,'CAFE_POST',str(exc),p.id,'ERROR')
        db.commit()
    return RedirectResponse('/admin',303)
